...
const ShareSheet: React.FC<{ 
  ...
  const handleDownload = async () => {
    try {
      ...
      link.href = blobUrl;
      link.download = `reel-${post.id}.${post.type === 'video' ? 'mp4' : 'jpg'}`;
      ...
      alert('Failed to download. The source might be restricted.');
    }
  };
...
export default function App() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const savedData: any = await get(STORAGE_KEY);
        if (savedData) {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      try {
        await set(STORAGE_KEY, { profiles, posts, myProfile, conversations });
      } catch (e) {
  ...
  }, [profiles, posts, myProfile, isLoading]);
    ...
    <div className="flex flex-col h-full bg-black">
      ...
      <div className="flex-1 p-4 space-y-6">
        ...
        <div className="space-y-2">
          ...
          <div 
            className={`flex items-center justify-between p-4 bg-zinc-900 rounded-2xl cursor-pointer ${!canInstall && 'opacity-50 cursor-not-allowed'}`}
          ...
          >
            ...
            {canInstall && <ChevronLeft size={20} className="rotate-180 text-zinc-500" />}
          </div>
          {!canInstall && (
        ...
        {view === 'create-post' && (
          <form onSubmit={handleCreatePost} className="space-y-4 pb-10">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[10px] text-zinc-500 px-1 mb-2">OR upload a file below</p>
              <div 
                onClick={() => document.getElementById('post-file')?.click()}
                className="w-full min-h-[200px] bg-gray-50 dark:bg-zinc-900 rounded-xl border-2 border-dashed border-gray-200 dark:border-zinc-800 flex flex-col items-center justify-center cursor-pointer overflow-hidden"
              >
                ...
                <input id="post-file" type="file" accept="image/*" className="hidden" onChange={e => handleFileUpload(e, setPostImageUrl)} />
              </div>
            </div>
            ...
            {postType === 'video' && (
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-zinc-500 px-1 mb-2">OR upload a file below</p>
                <div 
                  onClick={() => document.getElementById('post-video-file')?.click()}
                  className="w-full aspect-video bg-gray-50 dark:bg-zinc-900 rounded-xl border-2 border-dashed border-gray-200 dark:border-zinc-800 flex flex-col items-center justify-center cursor-pointer overflow-hidden"
                >
                  ...
                  <input id="post-video-file" type="file" accept="video/*" className="hidden" onChange={e => handleFileUpload(e, setPostVideoUrl)} />
                </div>
              </div>
            )}
